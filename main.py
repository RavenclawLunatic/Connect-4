'''
main.py is the game of connect 4 created so a
minimax algorithm could play it
3/11/22
...
def turn():
    ...
    try:
        ...
        if gameWon():
            if player == 1:
                textPrinting("Red won!", 500, 437.5, color(player))
            else:
                textPrinting("Yellow won!", 500, 437.5, color(player))
            pygame.display.update()
            time.sleep(3)
            running = False
        if player == 1:
        ...
        if player == 1:
            ...
            textPrinting("Yellow Turn", 500, 25, color(player))
        pygame.display.update()
    except:
        pass
